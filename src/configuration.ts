...
export const initializeStorageLocation = (location: string) => {
    if (location) {
        ...
        if (!fs.existsSync(extensionFilePath)) {
            ...
            fs.writeFileSync(extensionFilePath, '{"notes":[], "nextId":1, "spaces":[{"label": "time_std_space", "frame": {"label": "time_std_frame", "vector": null, "point": null}}], "frames":[{"label": "time_std_frame", "vector": null, "point": null}], "points":[], "vectors":[], "measurement_systems":[]}');
        }
    } else {
